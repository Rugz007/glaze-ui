...
>(({ className, children, ...props }, ref) => (
  <DialogPrimitive.Overlay
    className={cn(
      'data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in data-[state=closed]:fade-out fixed inset-0 z-50 bg-black/20 backdrop-blur-sm transition-all duration-100',
      className
    )}
    {...props}
    ref={ref}
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute top-4 right-4 rounded-sm opacity-70 transition-opacity hover:bg-primary-50 hover:opacity-100 disabled:pointer-events-none data-[state=open]:bg-primary-100">
        <X className="w-4 h-4" />
